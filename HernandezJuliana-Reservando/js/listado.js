...
Listado.prototype.buscarRestaurante = function(id){
    this.restaurantes.find(id => id === id);
    
    return "No se ha encontrado ningún restaurant";
}

Listado.prototype.obtenerCiudades = function() {
    let ciudades = this.restaurantes.map(index => index.ubicacion)

    let ciudadesFiltradas = resultadosFiltrados(ciudades);
...
Listado.prototype.obtenerRubro = function() {
    let rubros = this.restaurantes.map(index => index.rubro);
    
    let rubrosFiltrados = resultadosFiltrados(rubros);
...
function crearNuevosArreglos(data, atributo) {
    return data.map(elemento => elemento[atributo])
};

function resultadosFiltrados(data) { 
    return data.filter ((elem, index, self) => index === self.indexOf(elem));
};
